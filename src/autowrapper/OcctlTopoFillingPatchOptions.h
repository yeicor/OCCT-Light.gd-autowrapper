#ifndef OCCTLTOPOFILLINGPATCHOPTIONS_H
#define OCCTLTOPOFILLINGPATCHOPTIONS_H
...
class OcctlTopoFillingPatchOptions : public godot::RefCounted {
    GDCLASS(OcctlTopoFillingPatchOptions, godot::RefCounted)
protected:
...
public:
    ...
    int64_t p_next;
    Array edges;
    mutable std::vector<occtl_topo_filling_patch_edge_t> _cache_edges;
    int edge_count;
    Array points;
    ...
    occtl_topo_filling_patch_options_t to_c() const;
    static Ref<OcctlTopoFillingPatchOptions> from_c(const occtl_topo_filling_patch_options_t& val);
    void copy_from_c(const occtl_topo_filling_patch_options_t& val);
};

#endif // OCCTLTOPOFILLINGPATCHOPTIONS_H
