...
static func test_mesh_faces_with_box() -> String:
	...
	var face_ids = _collect_node_kind_ids(graph, OclCore.OCCTL_KIND_FACE)
	if face_ids.size() == 0:
...
static func test_mesh_edges_with_box() -> String:
	...
	var edge_ids = _collect_node_kind_ids(graph, OclCore.OCCTL_KIND_EDGE)
	if edge_ids.size() == 0:
...
static func test_mesh_vertices_with_box() -> String:
	...
	var vert_ids = _collect_node_kind_ids(graph, OclCore.OCCTL_KIND_VERTEX)
	if vert_ids.size() == 0:
